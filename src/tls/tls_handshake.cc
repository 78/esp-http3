...
static bool WriteExtension(BufferWriter* w, ExtensionType type, const uint8_t* data, size_t len) {
...
size_t BuildClientHello(const std::string& hostname, const uint8_t* client_random, const uint8_t* x25519_public_key,
    ...
    // 1. SNI extension
    {
        ...
        sni.WriteBytes(reinterpret_cast<const uint8_t*>(hostname.data()), hostname.size());
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kServerName, sni_data.data(), sni.Offset())) {
            return 0;
        }
    }
    ...
    // 2. Supported Groups extension
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kSupportedGroups, groups, sizeof(groups))) {
            return 0;
        }
    }
    ...
    // 3. Signature Algorithms extension
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kSignatureAlgorithms, sig_algs, sizeof(sig_algs))) {
            return 0;
        }
    }
    ...
    // 4. Supported Versions extension (ClientHello format)
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kSupportedVersions, versions, sizeof(versions))) {
            return 0;
        }
    }
    ...
    // 5. Key Share extension
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kKeyShare, key_share, ks.Offset())) {
            return 0;
        }
    }
    ...
    // 6. ALPN extension
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kALPN, alpn_data, sizeof(alpn_data))) {
            return 0;
        }
    }
    ...
    // 7. QUIC Transport Parameters extension
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kQUICTransportParameters, tp_data.data(), tp_len)) {
            return 0;
        }
    }
...
size_t BuildClientHelloWithPsk(const std::string& hostname, const uint8_t* client_random,
                               const uint8_t* x25519_public_key, const quic::TransportParameters& transport_params,
                               const PskParameters& psk, uint8_t* out, size_t out_len) {
    if (!psk.valid) {
        ESP_LOGW(TAG,
                 "BuildClientHelloWithPsk: PSK not valid, falling back to "
                 "full handshake");
        return BuildClientHello(hostname, client_random, x25519_public_key, transport_params, out, out_len);
    }
    ...
    // 8. PSK Key Exchange Modes extension (MUST come before pre_shared_key)
    {
        ...
        if (!WriteExtension(&ext_writer, ExtensionType::kPskKeyExchangeModes, psk_modes, sizeof(psk_modes))) {
            return 0;
        }
    }
    ...
    // 2. Compute hash of partial ClientHello (excluding binders_length and
    // binders)
    ...
    // RFC 8446: transcript includes everything up to but NOT including
    // binders_length
    ...
    // Calculate how many bytes to exclude from hash (binders_length +
    ...
    if (!WriteExtension(&ext_writer, ExtensionType::kPreSharedKey, psk_ext_data.data(), psk_writer.Offset())) {
        return 0;
    }
    ...
    // The transcript for binder includes everything up to (but not including) the
    // binder values
    BufferWriter writer(out, out_len);
    ...
    if (!quic::HkdfExpandLabel(early_secret, 32, binder_label, 10, empty_hash, 32, binder_key, 32)) {
    ...
    // binders_total_len = 2 (binders_length) + 1 (binder_length) + 32 (binder) =
    // 35
    ...
    ESP_LOGD(TAG, "PSK binder: hash_len=%zu, total_len=%zu, binders_excluded=%zu", hash_len, writer.Offset(),
             binders_total_len);

    // 4. Derive finished_key from binder_key
    const uint8_t* finished_label = reinterpret_cast<const uint8_t*>("finished");
    uint8_t finished_key[32];
    if (!quic::HkdfExpandLabel(binder_key, 32, finished_label, 8, nullptr, 0, finished_key, 32)) {
    ...
    ESP_LOGI(TAG, "Built ClientHello with PSK (ticket=%zu bytes, total=%zu bytes)", psk.identity.ticket.size(),
             writer.Offset());
...
size_t ParseHandshakeHeader(const uint8_t* data, size_t len, HandshakeType* msg_type, uint32_t* msg_len) {
    if (len < 4) {
        return 0;
    }
...
bool ParseServerHello(const uint8_t* data, size_t len, ServerHelloData* out) {
    ...
    if (legacy_version != kTlsLegacyVersion) return false;
    ...
    out->is_hello_retry_request = (std::memcmp(out->server_random, kHelloRetryRequestMagic, 32) == 0);
    ...
    while (reader.Offset() < ext_end) {
        ...
        if (ext_type == static_cast<uint16_t>(ExtensionType::kSupportedVersions)) {
            if (saw_supported_versions) return false;
            saw_supported_versions = true;
            if (ext_len == 2) {
                out->selected_version = (static_cast<uint16_t>(ext_data[0]) << 8) | static_cast<uint16_t>(ext_data[1]);
            } else {
                return false;
            }
        } else if (ext_type == static_cast<uint16_t>(ExtensionType::kKeyShare)) {
            if (saw_key_share) return false;
            saw_key_share = true;
            ...
            if (group != kX25519 || key_len != 32 || ks_reader.Remaining() != 32) return false;
            ...
            if (saw_pre_shared_key) return false;
            saw_pre_shared_key = true;
            // Pre-shared key extension in ServerHello contains only the selected
            // identity index
            if (ext_len != 2) return false;
            out->has_psk = true;
            out->selected_psk_identity = (static_cast<uint16_t>(ext_data[0]) << 8) | static_cast<uint16_t>(ext_data[1]);
        }

        reader.Skip(ext_len);
    }

    return reader.Offset() == ext_end && reader.Remaining() == 0 && saw_supported_versions && saw_key_share;
}
...
bool ParseEncryptedExtensions(const uint8_t* data, size_t len, EncryptedExtensionsData* out) {
    BufferReader reader(data, len);
    ...
    bool saw_alpn = false;
    bool saw_transport_parameters = false;
    size_t ext_end = reader.Offset() + extensions_len;
    while (reader.Offset() < ext_end) {
        uint16_t ext_type, ext_len;
        if (!reader.ReadUint16(&ext_type)) return false;
        ...
        if (ext_type == static_cast<uint16_t>(ExtensionType::kALPN)) {
            if (saw_alpn) return false;
            saw_alpn = true;
            // ALPN: 2-byte list length, then (1-byte name length, name)*
            if (ext_len < 4) return false;
            const uint16_t list_len = (static_cast<uint16_t>(ext_data[0]) << 8) | ext_data[1];
            const uint8_t name_len = ext_data[2];
            if (list_len != ext_len - 2 || name_len != ext_len - 3) return false;
            out->alpn.assign(reinterpret_cast<const char*>(ext_data + 3), name_len);
        } else if (ext_type == static_cast<uint16_t>(ExtensionType::kQUICTransportParameters)) {
            if (saw_transport_parameters) return false;
            saw_transport_parameters = true;
            if (!quic::ParseTransportParameters(ext_data, ext_len, &out->transport_params)) return false;
    ...
    return reader.Offset() == ext_end && reader.Remaining() == 0 && saw_alpn && saw_transport_parameters;
}
...
bool ParseCertificate(const uint8_t* data, size_t len, CertificateData* out) {
    ...
    if (out->certificate_request_context_len > 0) {
        if (!reader.Skip(out->certificate_request_context_len)) return false;
    }

    // Certificate List length (3 bytes)
    if (reader.Remaining() < 3) return false;
    uint32_t certs_len = (static_cast<uint32_t>(reader.Current()[0]) << 16) |
                         (static_cast<uint32_t>(reader.Current()[1]) << 8) | static_cast<uint32_t>(reader.Current()[2]);
    reader.Skip(3);

    if (reader.Remaining() != certs_len || certs_len == 0) return false;

    BufferReader certificates(reader.Current(), certs_len);
    while (certificates.Remaining() > 0) {
        // Certificate data length (3 bytes)
        if (certificates.Remaining() < 3) return false;
        uint32_t cert_len = (static_cast<uint32_t>(certificates.Current()[0]) << 16) |
                            (static_cast<uint32_t>(certificates.Current()[1]) << 8) |
                            static_cast<uint32_t>(certificates.Current()[2]);
        if (!certificates.Skip(3) || cert_len == 0 || certificates.Remaining() < cert_len) return false;

        CertificateEntry entry;
        entry.cert_data.assign(certificates.Current(), certificates.Current() + cert_len);
        out->certificates.push_back(std::move(entry));
        if (!certificates.Skip(cert_len)) return false;

        // Extensions length (2 bytes)
        uint16_t ext_len;
        if (!certificates.ReadUint16(&ext_len)) return false;
        if (!certificates.Skip(ext_len)) return false;
    }

    return !out->certificates.empty();
}
...
bool ParseCertificateVerify(const uint8_t* data, size_t len, CertificateVerifyData* out) {
    BufferReader reader(data, len);
    ...
    if (sig_len == 0 || reader.Remaining() != sig_len) return false;
...
bool ParseNewSessionTicket(const uint8_t* data, size_t len, NewSessionTicketData* out) {
    BufferReader reader(data, len);
