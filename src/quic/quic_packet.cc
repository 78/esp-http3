...
size_t BuildInitialPacket(const ConnectionId& dcid, const ConnectionId& scid, const uint8_t* token, size_t token_len,
                          uint64_t packet_number, const uint8_t* payload, size_t payload_len,
                          const CryptoSecrets& client_secrets, uint8_t* out, size_t out_len, size_t min_packet_size) {
    ...
    // Token length + token
    header_offset += EncodeVarint(token_len, header.data() + header_offset, header.size() - header_offset);
    if (token_len > 0) {
    ...
    size_t encrypted_len = AeadEncrypt(client_secrets.key.data(), client_secrets.iv.data(), packet_number, out,
                                       header_len, plaintext.data(), plaintext.size(), out + offset);
    if (encrypted_len == 0) {
        return 0;
    }
    ...
    if (!ApplyHeaderProtection(client_secrets.hp.data(), sample, out, out + pn_offset, pn_len)) {
        return 0;
    }
...
size_t BuildHandshakePacket(const ConnectionId& dcid, const ConnectionId& scid, uint64_t packet_number,
                            const uint8_t* payload, size_t payload_len, const CryptoSecrets& client_secrets,
    ...
    header_offset += EncodeVarint(length_val, header.data() + header_offset, header.size() - header_offset);
    ...
    size_t header_len = offset;
    size_t encrypted_len = AeadEncrypt(client_secrets.key.data(), client_secrets.iv.data(), packet_number, out,
                                       header_len, payload, payload_len, out + offset);
    if (encrypted_len == 0) {
        return 0;
    }
    offset += encrypted_len;
...
size_t Build1RttPacket(const ConnectionId& dcid, uint64_t packet_number, bool spin_bit, bool key_phase,
                       const uint8_t* payload, size_t payload_len, const CryptoSecrets& app_secrets, uint8_t* out,
                       size_t out_len) {
    ...
    // Encrypt
    size_t encrypted_len = AeadEncrypt(app_secrets.key.data(), app_secrets.iv.data(), packet_number, out, header_len,
                                       payload, payload_len, out + offset);
    if (encrypted_len == 0) {
        return 0;
    }
    offset += encrypted_len;
    ...
    // Apply header protection
    const uint8_t* sample = out + pn_offset + 4;
    if (!ApplyHeaderProtection(app_secrets.hp.data(), sample, out, out + pn_offset, pn_len)) {
        return 0;
    }
...
bool ParsePacketHeader(const uint8_t* data, size_t len, size_t expected_dcid_len, PacketInfo* info) {
    if (len < 1) {
        return false;
    }
    ...
    if (info->is_long_header) {
        ...
        info->long_header.version = (static_cast<uint32_t>(data[1]) << 24) | (static_cast<uint32_t>(data[2]) << 16) |
        ...
        if (info->long_header.IsInitial()) {
            ...
            if (token_len > 0) {
                info->token.assign(reader.Current(), reader.Current() + token_len);
            }
...
size_t DecryptInitialPacket(uint8_t* packet, size_t packet_len, const CryptoSecrets& server_secrets, int64_t largest_pn,
                            PacketInfo* info, uint8_t* payload_out, size_t payload_out_len) {
    ...
    BufferReader reader(packet + info->header_length, packet_len - info->header_length);
    ...
    size_t pn_offset = info->header_length + (packet_len - info->header_length - reader.Remaining());

    // Remove header protection
    size_t pn_len;
    if (!RemoveHeaderProtection(server_secrets.hp.data(), packet, packet_len, pn_offset,
                                true,  // long header
                                &pn_len)) {
    ...
    if (ciphertext_len > payload_out_len + 16) {
        ESP_LOGW(TAG, "DecryptInitial: ciphertext too large (%zu > %zu)", ciphertext_len, payload_out_len + 16);
        return 0;
    }

    // Validate that the packet fits in the UDP datagram
    if (header_len + ciphertext_len > packet_len) {
        ESP_LOGW(TAG, "DecryptInitial: packet extends beyond datagram (%zu + %zu > %zu)", header_len, ciphertext_len,
                 packet_len);
        return 0;
    }

    size_t decrypted_len = AeadDecrypt(server_secrets.key.data(), server_secrets.iv.data(), info->packet_number, packet,
                                       header_len, packet + header_len, ciphertext_len, payload_out);
...
size_t DecryptHandshakePacket(uint8_t* packet, size_t packet_len, const CryptoSecrets& server_secrets,
                              int64_t largest_pn, PacketInfo* info, uint8_t* payload_out, size_t payload_out_len) {
    ...
    if (!info->is_long_header || !info->long_header.IsHandshake()) {
        ESP_LOGW(TAG, "DecryptHandshake: not a Handshake packet (first_byte=0x%02x)", packet[0]);
        return 0;
    }
    ...
    // Remove header protection
    size_t pn_len;
    if (!RemoveHeaderProtection(server_secrets.hp.data(), packet, packet_len, pn_offset, true, &pn_len)) {
    ...
    if (ciphertext_len > payload_out_len + 16) {
        ESP_LOGW(TAG, "DecryptHandshake: ciphertext too large (%zu > %zu)", ciphertext_len, payload_out_len + 16);
        return 0;
    }

    // Validate that the packet fits in the UDP datagram
    if (header_len + ciphertext_len > packet_len) {
        ESP_LOGW(TAG, "DecryptHandshake: packet extends beyond datagram (%zu + %zu > %zu)", header_len, ciphertext_len,
                 packet_len);
        return 0;
    }
...
size_t Decrypt1RttPacket(uint8_t* packet, size_t packet_len, size_t dcid_len, const CryptoSecrets& server_secrets,
                         int64_t largest_pn, PacketInfo* info, uint8_t* payload_out, size_t payload_out_len) {
    if (!server_secrets.valid || packet_len < 20) {
        return 0;
    }
    ...
    // Header protection covers the Key Phase and packet-number-length bits.
    // Publish the unmasked byte so the connection can detect a peer key
    // update only after authenticated decryption succeeds.
    info->short_header.header_byte = packet[0];
...
bool ParseRetryPacket(const uint8_t* packet, size_t packet_len, const ConnectionId& original_dcid, PacketInfo* info,
    ...
    if (!VerifyRetryIntegrityTag(original_dcid.Data(), original_dcid.Length(), packet, packet_len)) {
        return false;
    }
