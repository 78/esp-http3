...
void AckManager::OnPacketReceived(uint64_t pn, uint64_t recv_time_us, bool ack_eliciting) {
    ...
    // Every authenticated packet advances packet-number reconstruction and
    // may appear in a future ACK range. ACK-only packets must not themselves
    // schedule an ACK, otherwise peers create an ACK loop.
    if (ack_eliciting) {
...
int64_t AckManager::GetLargestReceived() const { return largest_received_; }
 ...
 */
static std::vector<std::pair<uint64_t, uint64_t>> GetAckRanges(const std::set<uint64_t>& received_packets) {
...
SentPacketTracker::SentPacketTracker() { Reset(); }

void SentPacketTracker::Reset() {
    Http3Vector<SentPacketInfo>().swap(sent_packets_);
    next_packet_number_ = 0;
    largest_acked_ = -1;
    latest_rtt_us_ = 0;
}

void SentPacketTracker::OnPacketSent(uint64_t pn, uint64_t sent_time_us, size_t sent_bytes, bool ack_eliciting,
                                     Http3Vector<uint8_t> frames, uint64_t stream_id) {
    SentPacketInfo info;
...
bool SentPacketTracker::OnAckReceived(uint64_t largest_acked, uint64_t ack_delay, uint64_t first_ack_range,
                                      const std::vector<std::pair<uint64_t, uint64_t>>& ack_ranges,
                                      uint64_t current_time_us, size_t* newly_acked_bytes) {
    *newly_acked_bytes = 0;
    ...
    for (auto& pkt : sent_packets_) {
        ...
        if (covered) {
            ...
            *newly_acked_bytes += pkt.sent_bytes;
            ...
            Http3Vector<uint8_t>().swap(pkt.frames);
...
void SentPacketTracker::PruneOldPackets() {
    ...
    sent_packets_.erase(std::remove_if(sent_packets_.begin(), sent_packets_.end(),
                                       [](const SentPacketInfo& p) { return p.acknowledged || p.lost; }),
                        sent_packets_.end());
}
...
size_t SentPacketTracker::ClearStreamFrames(uint64_t stream_id) {
    ...
    for (auto& pkt : sent_packets_) {
        ...
        if (!pkt.acknowledged && !pkt.lost && pkt.in_flight && pkt.stream_id == stream_id && !pkt.frames.empty()) {
