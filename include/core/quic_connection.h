...
struct QuicConfig {
    ...
    // TLS peer authentication. The trust anchor is a DER-encoded CA
    // certificate and is owned by this configuration. Certificate chain,
    // validity period, hostname, CertificateVerify, and Finished are checked
    // before application keys are accepted. The development override skips
    // chain/date/hostname validation only; leaf usage, CertificateVerify, and
    // Finished remain mandatory.
    ...
    uint32_t max_stream_data = quic::defaults::kInitialMaxStreamDataBidiRemote;  ///< Per-stream flow
                                                                                 ///< control limit
    uint32_t max_streams_bidi = quic::defaults::kInitialMaxStreamsBidi;          ///< Max concurrent bidirectional
                                                                                 ///< streams
    uint32_t max_streams_uni = quic::defaults::kInitialMaxStreamsUni;            ///< Max concurrent unidirectional
                                                                                 ///< streams
    ...
    // External X25519 keypair (for reuse across connections to speed up
    // reconnection) If both are non-null, the keypair will be reused instead of
    // generating new one. client_random is still regenerated each connection for
    // security.
    ...
    // If session_ticket is non-empty and psk is non-empty, PSK resumption will be
    // attempted. Ticket will be validated against lifetime before use.
...
using OnStreamDataCallback = std::function<void(int stream_id, const uint8_t* data, size_t len, bool fin)>;
...
/// Called when connection becomes writable (MAX_DATA received or ACK frees
/// congestion window) Upper layer should retry any blocked writes.
...
/// Called when peer asks us to stop sending request data (STOP_SENDING
/// received) error_code is the application error code from the frame
...
public:
   ...
   *
   * @param data Pointer to received data (mutable, will be modified in-place
   * for decryption)
   ...
   */
    int SendRequest(const std::string& method, const std::string& path,
                    const std::vector<std::pair<std::string, std::string>>& headers = {}, const uint8_t* body = nullptr,
                    size_t body_len = 0);
   ...
   */
    ...
    /**
   * @brief Write data to an open stream (synchronous, may be flow-control
   * limited)
   *
   * @param stream_id Stream ID from OpenStream()
   * @param data Data to send
   * @param len Length of data
   * @return Number of bytes written (may be less than len if flow-control
   * limited) 0 if flow-control blocked (no bytes could be sent) -1 on error
   * (stream doesn't exist or connection error)
   *
   ...
   * @param error_code Application error code (default: H3_REQUEST_CANCELLED =
   * 0x10c)
   ...
   */
    ...
    /// Set callback for connection becoming writable (MAX_DATA or ACK frees
    /// window)
   ...
   * - Server must not have disabled migration (IsMigrationAllowed() returns
   * true)
   ...
   *
   * Returns false if the server sent disable_active_migration transport
   * parameter.
   *
